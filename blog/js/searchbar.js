...
$(function(){
    ...
    $tip.show();
	setTimeout( function(){$tip.fadeOut();}, ( time * 1000 ) ); //提示信息消失
	...
	function highlight(){
		...
		if($.trim(searchText)=="" || $.trim(searchText)=='.'){
			...
			showTips("请输入查找关键字",_searchTop,1.5,_searchLeft);
		...
		if (!regExp.test(content)) {
			showTips("没有找到要查找的内容",_searchTop,1.5,_searchLeft);
	...
	resizePre = function(){
		...
		var y = $('.fa-hover');
		y.css('clear', 'none');
		$('.fa-hover').css('float', 'left');
		if (x > 980) {
			$('.fa-hover').css('width', '25%');
			y.each(function(i, v){
				if (i % 4 == 0) {
					$(v).css('clear', 'left');
				}
